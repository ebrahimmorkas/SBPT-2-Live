...
<?php
        ...
        if(isset($_SESSION['login']))
        {
            ...
            if($numOfRows)
            {
                ...
                function youtubeOnlyAudio($videoid)
                {
                    echo '
                        <center>
                            <div class="videoplyr" id="onlyAudio">
                ...
                else
                {
                    for($i = 0 ; $i < count($arrayOfIndex) ; $i++)
                    {
                        ...
                        if($selectServerOptionRow['video'] == 1)
                        {
                            ...
                            // Alternative to str_contains is '$str1 = stripos($link, 'live', 20);'. If str_contains does not work copy this as it is and the conditions will be
                            // if($str1 == 20)
                            // {
                            //     Video is live
                            // }
                            ...
                            //     video is not live
                            // }
                            // On index 20 there will be live word if the video would be live and this functions indexing starts from 0.
                            // Do this for user side as well

                            if(str_contains($link, 'live'))
                            {
                                // Video is live video
                                // echo "Both Audio And Video And Not Live";
                                $videoID = extractVideoIDofLiveVideo($link);
                                youtubeVideoAndAudio($videoID);
                            }
                            else
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            const player = Plyr.setup('.js-playerOnlyAudio');
        });
    </script>
    ...
    <!-- <script>
        ...
        one.addEventListener('click', () => {
            ...
            if(i % 2 == 0)
            {
                ...
                let mobile = document.getElementById('nMobile');
                // mobile.classList.remove('show');
                mobile.classList.remove("navbar-collapse");
                mobile.classList.remove("collapse");
                mobile.classList.remove("show");
                mobile.classList.add("collapse");
                mobile.classList.add("navbar-collapse");
                one.setAttribute("aria-expanded", "false");
                
            }
            else
            {
                one.classList.remove("collapsed");
            }
            i++;
        })
    </script> -->
